...
from fsspec.implementations.ia import ENV_ACCESS_KEY, ENV_SECRET_KEY, InternetArchiveFileSystem, load_ia_credentials
...
#
# `InternetArchiveFileSystem` lives in fsspec (fsspec/implementations/ia.py, protocol "ia"): an
# HTTPFileSystem reading https://archive.org/download/<identifier>/<filename>, with credentials from
# the ia.ini that `ia configure` writes. fsspec registers the protocol itself, so fsspec_open(),
# url_to_fs() and therefore main(), --fetch and input_basename() take ia:// with no code here. Until
# a fsspec release ships it, the branch behind the upstream PR is installed on top of the released
# fsspec (see pyproject.toml). What stays in warc2zip is the CLI's explanation of a refused item.
...
def format_ia_permission_error(input_file, credentials=None):
    ...
    return "\n".join(
        [
            ...
            "",
            f"or set {ENV_ACCESS_KEY} and {ENV_SECRET_KEY}. Public items need neither; check the item name.",
